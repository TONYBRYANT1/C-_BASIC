...
//	//printf("%c\n", sh);
//	char arr[] = "I Have Come Here TO Chew Bubblegum And Kick Ass , And I` All Out Of Bubblegum";
//	int i = 0;
//	while (arr[i])
//	{
//		if (isupper(arr[i]))
...
//	int arr2[5] = { 0 };        // 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00  
//	strcpy(arr2, arr1);         // 01 看到\0停止
////"int *" 类型的实参与 "char *" 类型的形参不兼容
//}
...
//9.内存操作函数:memcpy(Copies characters between buffers 在缓冲区拷贝字符)------void *memcpy( void *dest, const void *src, size_t count );
//9.0.0:                                                                                   ( New buffer),( Buffer to copy from),(Number of characters to copy字节数)
//9.0.1:memcpy returns the value of dest.
//9.0.2:memcpy();-----处理不重叠的内存拷贝 
// 
// 
//9.1:应用
//struct S
...
// 
//9.2:自定义函数实现:
//void* my_memcpy(void* dest,const void* src, size_t count)//------------------------有问题
//{
//	void* ret = dest;
//	assert(dest && src);
//	while (count--)
...
//10.memmove();-----处理重叠/不重叠的内存拷贝
//10.1:应用
//10.2:自定义函数实现:------------------------------------------------------------有未解决的问题
//version 1:
//void* my_memmove(void* dest, const void* src, size_t count)//void* 类型要强制类型转换才能使用
//{
//	void* ret = dest;
//	assert(dest && src);
//	if (dest < src)
...
//	int arr1[] = { 1,2,5,6,7 };
//	int arr2[] = { 1,2,3,4,5 };
//	int ret=memcmp(arr1, arr2, 9);//9个字节
//	printf("%d", ret);
//	return 0;
//}
...
//{
//	char arr1[20] = "abc";
//	memset(arr1, '#', 10);//10个字节
//	printf("%s", arr1);
//	int arr2[10] = { 0 };
//	memset(arr2, 1, 40);//40个字节
//	int i = 0;
//	for (i = 0; i < 10; i++)
//	{
//		printf("%d", arr2);
